...
export class HttpService {
  ...
  public updatePass(token: string, username: string, pass: string){
    const headers = new HttpHeaders().set("Authorization", token)
                                     .set("Content-Type", "application/x-www-form-urlencoded");
    const body = '{password:"' + pass + '"}';
    console.log(body);
    const obj = JSON.stringify("password:" + pass);
    console.log(obj);
    return this.http.patch(this.baseurl + '/users/' + username, obj, {headers: headers, observe: 'response'});
  }
